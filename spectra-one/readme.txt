...
* Fix: Used dynamic tag name from block attributes instead of hardcoded selectors in header rendering.
...
= 1.1.3 =
* Fix: Container Full Width not working correctly with the FSE templates.
* Fix: PHP error when Plugin Detective dashboard is visited from wp-admin > tools > Plugin Detective

= 1.1.2 =
* Fix: PHP error when Gutenberg plugin is activated.

= 1.1.1 =
* Fix: CSS grid and width issue with the Spectra plugin.

= 1.1.0 =
* Fix: Compatibility with WordPress 6.5.

= 1.0.9 =
* Update: Merged Spectra One and Spectra Plugin settings inside Editor.
* Fix: Heading color compatibility with Spectra plugin.
